...
def plot_fit(
    ...
    label_fit = args.argscan(
        label_fit, self.label_fit, "fit ({nP}P, {nZ}Z, Rsq ={Rsq:.2e})"
    )
                ...
                f = np.sort(
                    np.concatenate(
                        [
                            ...
                            logspaced(
                                np.min(fitter.F_Hz[fitter.F_Hz > 0])
                                / plot_data_scale_low,
                                fitter.F_nyquist_Hz,
                                N_points // 2,
                            ),
            ...
            f = np.sort(
                np.concatenate(
                    [
                        np.linspace(0, F_max_Hz, N_points // 2),
                        logspaced(
                            np.min(fitter.F_Hz[fitter.F_Hz > 0]) / plot_data_scale_low,
                            F_max_Hz,
                            N_points // 2,
                        ),
                        fitter.F_Hz[intersperse // 2 :: intersperse],
    ...
    def finalize():
        ...
        axB.mag.legend(ncol=1, fontsize=8, loc="best")
