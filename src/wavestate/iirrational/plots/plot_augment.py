...
def plot_augment_data(
    ...
    def plot(
        ...
        if with_error:
            ...
            if ax_mag:
                ...
                ax_mag.errorbar(
                    ...
                    abs_h[select],
                    yerr = (
                        abs_h[select] - np.maximum((abs_h / (1 + 1 / W))[select], np.min(abs_h[select]) / 100),
                        np.minimum((abs_h * (1 + 1 / W))[select], np.max(abs_h[select]) * 100) - abs_h[select],
                    ),
                    color = color,
                    label = label,
                    **kwargs
            ...
            if ax_phase:
                ...
                ax_phase.errorbar(
                    ...
                    angle_h[select],
                    (
                        angle_h[select] - np.maximum((angle_h - 180/np.pi / W)[select], -180),
                        np.minimum((angle_h + 180/np.pi / W)[select], 180) - angle_h[select],
                    ),
                    color = color,
                    label = label,
                    **kwargs
        ...
        if np.count_nonzero(fitter.F_Hz < midpt) < .65 * len(fitter.F_Hz):
            db.scale_log = False
        else:
            db.scale_log = True
...
def plot_augment_fit(
    ...
    label_default = args.argscan(label, self.label_fit, 'fit ({nP}P, {nZ}Z, Rsq ={Rsq:.3e})')
    ...
    def plot(
        ...
        if with_error and isinstance(fitter, fitters_ZPK.MultiReprFilterZ):
            ...
            ax_mag.fill_between(
                f[select],
                np.maximum((abs(h) / (1 + fitter.xfer_fit_error.mag_rel2))[select], np.min(abs(h)) / 10),
                np.minimum((abs(h) * (1 + fitter.xfer_fit_error.mag_rel2))[select], np.max(abs(h)) * 10),
                color = db.mline.get_color(),
                alpha = .3,
            )
            if ax_phase:
                ax_phase.fill_between(
                    f[select],
                    np.minimum((np.angle(h, deg = True) + 180/np.pi * fitter.xfer_fit_error.phase)[select], 180),
                    np.maximum((np.angle(h, deg = True) - 180/np.pi * fitter.xfer_fit_error.phase)[select], -180),
                    color = db.pline[0].get_color(),
                    alpha = .3,
                )
                db.min_fit_err_lim = np.min(abs(h)) * max(1/3, np.min(1 / (1 + fitter.xfer_fit_error.mag_rel2)[select]))
                db.max_fit_err_lim = np.max(abs(h)) * min(3, np.min((1 + fitter.xfer_fit_error.mag_rel2)[select]))
        else:
...
def plot_augment_residuals(
    ...
    def plot(
        ...
        if np.count_nonzero(f < midpt) < .65 * len(f):
            db.scale_log = False
        else:
            db.scale_log = True
