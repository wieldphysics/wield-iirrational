...
def mapcheck_delay_s(aid, aname, val):
    ...
    if val < val_min:
        raise ArgumentError(
            ("Argument 'delay_s'={} must be larger than 'delay_s_min'={}").format(
                val, val_min
            )
        )
    ...
    if val_max is not None:
        if val > val_max:
            raise ArgumentError(
                ("Argument 'delay_s'={} must be larger than 'delay_s_max'={}").format(
                    val, val_max
                )
            )
    elif val > val_min:
...
def mapcheck_total_degree_min(aid, aname, val):
    ...
    val = int(val)
    if val < 0:
        raise ArgumentError("Argument '{}'=val must not be negative".format(aname, val))
    return val
