...
class ResultsAidAdv(results_aid.ResultsAid):
    ...
    def __init__(self, fit_aid, **kwargs):
        ...
        for name, func in self._investigations.items():
            setattr(
                self, name,
                (lambda func: lambda *args, **kwargs: func(self, *args, **kwargs))(func),
            )
        return

    def investigate_fit_plot(
            self,
            fname = None,
            xscale = 'log_zoom',
            **kwargs
    ...
    def investigate_order_plot(
        ...
        for idx in reversed(range(len(fbo))):
            ...
            if not lbo[idx]:
                continue
            axB.plot_fit(fit, label = (
                'order: {}, ChiSq: {:.3e}'
            ).format(fit.order, fit.residuals_average),
            )
        if fname is not None:
            axB.save(fname)
        else:
