...
def config_load(aid, args, kwfull, mode):
    ...
    for cfname in reversed(cfiles):
        ...
        if fB_conf.ftype == 'csv':
            raise arguments.ArgumentError(
                "Configuration files may not be csv"
            )
        elif fB_conf.ftype == 'ini':
            raise arguments.ArgumentError(
                "ini files not supported"
            )
        cgroup_use = cgroup
        ...
        if len(used_cgroups) == 0:
            print((
                "No config groups found in config file {}"
            ).format(fB_conf.fname), file = sys.stderr)
...
def data_load(aid, args, kwfull, mode, confB):
    ...
    try:
        ...
        raise arguments.ArgumentError((
            "Unrecognized file type for data file {}"
        ).format(dfile))
    ...
    if fB_data.ftype == 'csv':
        ...
        if len(used_dgroups) == 0:
            print((
                "No data groups found in data file {}"
            ).format(fB_data.fname), file = sys.stderr)
            ...
            print((
                "Warning: multiple possible data groups found in data file {}"
            ).format(fB_data.fname), file = sys.stderr)
...
def data_assemble(aid, kwfull, dataB):
    def pull_data_element(elname):
        ...
        if len(used) > 1:
            raise DataError((
                "multiple possible redundant elements in data group: {}"
            ).format(used))
        elif len(used) > 0:
    ...
    try:
        ...
        if Xc is not None:
            ...
            if not np.iscomplexobj(xfer):
                raise(RuntimeError("Xfer found, but does not appear to be complex {}".format(xfer)))
...
def check_pzargs(args):
    ...
    msg = ("Error, must specify some poles if -{0} flag given, "
...
def IIRrationalV2fit(argv = None, **kwargs):
    ...
    if ofile is not None:
        try:
            ...
            raise arguments.ArgumentError((
                "Unrecognized file type for output file {}"
            ).format(ofile))
        if not overwrite and os.path.exists(fB_output.fname):
            print((
                "Error: output file {} already exists"
            ).format(ofile), file = sys.stderr)
            raise SystemExit(1)
    ...
    if choose in ['shell', 'prompt']:
        ...
        try:
            ...
            print("Error: order choice not an int, using baseline order", file = sys.stderr)
        else:
            results.choose(order)
    ...
    if refine:
        print('%%% add or adjust the following poles and zeros to bootstrap the fit and refine. Disable this message with --no-refine')
