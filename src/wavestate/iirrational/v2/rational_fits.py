...
def fit_cheby_base(
    ...
    if False:
        ...
        aid.log_info(1, "discarding {n} zeros: {Z}".format(n = np.count_nonzero(select), Z = Zc_discard))

        Pc = rat_fitter.ZPKrep.poles.c
        select = (Pc.imag - 2 * abs(Pc.real) > 1.4 * rat_fitter.F_max_Hz)
        Pc_use = Pc[~select]
        Pc_discard = Pc[select]
        aid.log_info(1, "discarding {n} poles: {P}".format(n = np.count_nonzero(select), P = Pc_discard))
...
def fit_disc(aid):
    ...
    aid.log(1, "Initial Order: (Z={0}, P={1}, Z-P={2})".format(
        len(fitter.zeros),
        len(fitter.poles),
