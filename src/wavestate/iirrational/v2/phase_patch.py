...
def phase_patch(aid):
    ...
    Wthresh = Wsort[int(len(aid.fitter.W) * 0.2)]
    ...
    if aid.hint("trust_SNR"):
        ...
        with aid.log_heading("sample variance (from magnitude)"):
            ...
            W = fitter_X.W
            meanWeight = (np.sum(W ** 4) / np.sum(W ** 2)) ** 0.5
            cRvar = (np.sum((abs(R) - 1) ** 2 * W ** 2) / np.sum(W ** 2)) ** 0.5
            aid.log_debug(5, "variance", 1 / cRvar, meanWeight)
            weight_scale_true = meanWeight * cRvar
            weight_scale = weight_scale_true ** 0.5
    ...
    with aid.log_heading("Phase patching"):
        ...
        separations = list(
            np_utils.logspaced(
                3, len(fitter_X.F_Hz) / 2, int(1 + np.log(len(fitter_X.F_Hz)))
            )
        )
        ...
        while separations:
            ...
            shift_err = (
                weight_scale
                * ((cFd[sep:] - cFd[0:-sep]) / (cW[sep:] - cW[0:-sep]) / (2 * sep))
                ** 0.5
            )
            ...
            for sec_L, sec_R in np_utils.generate_sections(shift + shift_err < 0):
                ...
                try:
                    select2BW = (f_local < loc_Hz + 2 * BW_Hz) & (
                        f_local > loc_Hz - 2 * BW_Hz
                    )
    ...
    def investigate_phase_patch_plot(self, fname=None, xscale="log_zoom", **kwargs):
        ...
        axB = plots.plot_fitter_flag(
            fitter=phase_fixes[0].fitter_X, xscale=xscale, **kwargs
        )
    ...
    def investigate_phase_patch_stats_plot(resaid, fname=None, xscale="log_zoom"):
        """
    ...
    aid.investigations[
        "investigate_phase_patch_stats_plot"
    ] = investigate_phase_patch_stats_plot
