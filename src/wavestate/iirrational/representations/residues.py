...
class ChebyResiduesHelper(object):
    ...
    def residue_of(self, polemask):
        ...
        an = np.atleast_1d(
            cheby.chebfromroots(self.poles[polemask])
        )
        ...
        for m in range(mult, 0, -1):
            ...
            val = self.rot_p**(m + self.orderdiff) * ((
                cheby.chebval(pavg, bn ) / cheby.chebval(pavg, an)
            ) / factorial(mult - m)) / (self.scale)**(m + self.orderdiff)
...
def ZPK2residues(
    ...
    while ppairs:
        ...
        if Nd >= 2:
            ...
            #TODO, make the derivatives work up to some order
            R_2d = (
                N_2d - D_2d * N_0d / D_0d - 2 * R_1d * D_1d
            ) / D_0d
            eval_2d = gain * R_2d / 2
        ...
        if Nd == 0:
            ...
            raise RuntimeError("Residues with poles of mult>3 not implemented"
    ...
    if len(zeros) < len(poles):
        ...
        return residue_list, (tuple(helper.k_zeros), (), np.asarray(helper.k_gain).item())
...
def ZPK2residues_scipy(
    ...
    for idx_start, idx_end in zip(
        np.argwhere(same_starts | same_shift),
        np.argwhere(same_ends | same_shift)
    ):
...
def residues2ZPK(
    ...
    if ZPKadd is None:
        ...
        b = cheby.chebmul(
            cheby.chebfromroots(poles_scaled),
            cheby.chebfromroots(Z) * K
        )
