...
def rational_disc_fit(
    ...
    if nyquist_final_Hz is not None and nyquist_final_Hz <= 0:
        raise RuntimeError(
            "nyquist_final_Hz must be positive for Z domain or None" " for Sf domain."
        )
...
def ratdisc_single(
    ...
    fitter.match_pair_iter(Q_rank_cutoff=0.4, zeros_first=False)
    fitter.fit_poles()
    fitter.fit_zeros()
    fitter.match_pair_iter(Q_rank_cutoff=0.4)
    fitter.fit_poles()
    fitter.fit_zeros()
