...
def add_resrank(
    ...
    aid = ensure_aid(aid)
    dual_b.idx_min = np.searchsorted(
        aid.fitter.F_Hz, dual_b.F_loc_Hz - relupdown * dual_b.Q_rank_BW
    )
    dual_b.idx_max = np.searchsorted(
        aid.fitter.F_Hz, dual_b.F_loc_Hz + relupdown * dual_b.Q_rank_BW
    )
    ...
    h = TFmath.polyvalfromroots(Xsf, [dual_b.z]) / TFmath.polyvalfromroots(
        Xsf, [dual_b.p]
    )
    # aid.log(abs(h))
    ...
    dual_b.res_mod_tot = np.sum(TFmath.abs_sq(res_mod)) / (
        dual_b.idx_max - dual_b.idx_min
    )
    return dual_b
...
def order_reduce(
    ...
    for b in duals:
        ...
        if b.Q_rank < Q_rank_cutoff:
            ...
            F_mean_Hz = (aid.fitter.F_Hz[F_idx_high] - aid.fitter.F_Hz[F_idx_low]) / (
                F_idx_high - F_idx_low
            )
    ...
    def nearest_idx(lst_1, lst_2, num_first=True):
        ...
        for idx_1, pair_1 in enumerate(lst_1):
            ...
            for idx_2, pair_2 in enumerate(lst_2):
                ...
                dist = (ang_diff ** 2 + 1 * mag_diff ** 2) ** 0.5
                if dist < dist_nearest:
                    idx_nearest = idx_2
                    dist_nearest = dist
    ...
    while True:
        ...
        def closure(ORb):
            def plot_duals(fitter):
                ...
                plots.plot_ZP_grab(fitter, ORb.duals_used, axB=axB, color="black")
                plots.plot_ZP_grab(fitter, ORb.duals_rejected, axB=axB, color="blue")
                return axB
