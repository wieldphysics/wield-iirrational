...
class PolyFilterAlgorithmsBase(DataFilterBase):
    ...
    def Q_rank_Snative(self, p, z):
        ...
        return (
            abs(p_soft - z_soft) * (1 / p_soft.real ** 2 + 1 / z_soft.real ** 2) ** 0.5
        )
...
class PolyFilterAlgorithms(PolyFilterAlgorithmsBase):
    ...
    def fit_SVD(self, modify=True, residuals=0, **kwargs):
        ...
        if residuals == 0:
            ...
            A_z = self.V_b * (self.W / (self.h_b / self.h_delay)).reshape(-1, 1)
        elif residuals < 0:
            A_p = self.V_a * (self.W / (self.h_delay * self.h_a)).reshape(-1, 1)
