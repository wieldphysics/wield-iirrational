...
def plot_ZP(
    ...
    def plot_poles(ax):
        ...
        if db_nyquist:
            ...
            r_pos = np.maximum(
                -10
                * np.log10(
                    utilities.BWz(r[select], fitter.F_nyquist_Hz / F_largest_Hz)
                ),
                0,
            )
            r_neg = np.maximum(
                -10
                * np.log10(
                    utilities.BWz_neg(r[~select], fitter.F_nyquist_Hz / F_largest_Hz)
                ),
                0,
            )
        else:
    ...
    axB.ax_poles.set_xticklabels(
        ["{0:.1f}".format(f) for f in np.linspace(0, fitter.F_nyquist_Hz, num)]
    )
    axB.ax_dual.set_xticks(np.linspace(0, np.pi, num))
    axB.ax_dual.set_xticklabels(
        ["{0:.1f}".format(f) for f in np.linspace(0, fitter.F_nyquist_Hz, num)]
    )
    axB.ax_zeros.set_xticks(np.linspace(0, np.pi, num))
    axB.ax_zeros.set_xticklabels(
        ["{0:.1f}".format(f) for f in np.linspace(0, fitter.F_nyquist_Hz, num)]
    )
