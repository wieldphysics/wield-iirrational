...
This is from https://www.embeddedrelated.com/showarticle/927.php
"""
...
def zpk2ss_realmodal(
    zeros, poles, k, residuals=None, imag_limit=None, as_matrices=False
):
    """
    convert zeros poles k representation to state-space
...
class PadeExponential(object):
    def __init__(self, p, q, f0=1.0, rootsorter=None):
        ...
        self.rootsorter = rootsorter if rootsorter is not None else lambda z: np.imag(z)
    ...
    def get_lti_realmodal(self, imag_limit=None):
        ...
        """
        return zpk2ss_realmodal(
            self.zeros, self.poles, self.k, self.residuals, imag_limit=imag_limit
        )
    ...
    def lti_sscascade(self):
        ...
        by sorting poles and zeros
        """
        return zpk2ss_cascade(
            self.zeros, self.poles, self.k, rootsorter=self.rootsorter
        )
