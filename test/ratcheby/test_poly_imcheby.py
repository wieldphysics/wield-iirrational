...
from IIRrational.fitters_rational.polynomials import imchebychev
import numpy as np
...
def test_imcheby_4roots_1scale():
    X_scale = 1
    roots = [-.8 * X_scale, 0+.5j * X_scale, 0-.5j * X_scale, -.5 * X_scale]
    c, lnG = imchebychev.fromroots_lnG(roots, X_scale = X_scale)

    F = np.linspace(0, X_scale, X_scale)
    X = 1j * F

    y_main, y_main_lnG = imchebychev.valfromroots_lnG(X, roots, X_scale = X_scale, lnG = lnG)
...
def test_imcheby_4roots():
    ...
    assert_roots_close(
        roots,
        imchebychev.roots(imchebychev.fromroots_lnG(roots, X_scale = X_scale)[0], X_scale = X_scale),
    )
...
def test_imcheby_3roots():
    X_scale = 100
    roots = [-.8 * X_scale, 0+.5j * X_scale, 0-.5j * X_scale]
    assert_roots_close(
        roots,
        imchebychev.roots(imchebychev.fromroots_lnG(roots, X_scale = X_scale)[0], X_scale = X_scale),
    )
