...
def build_model(theta = 0):
    ...
    k = 2 * np.pi / 1064e-9
    Msrm = mirror('Msrm', T = .324, k = k)
    Mitm = mirror('Mitm', T = .0148, k = k, P_incA = 200e3)
    ...
    sys1 = StateSpaceDense.join('sys', [
        Sarm, Ssrc, Msrm, Mitm, Metm, SsrcD, Ddarm, DsrmQ, DsrmP, FMitm, FMetm,
    ])
...
def test_IFO_model(tpath_join):
    ...
    F_Hz = logspaced(1, model.FSR_Hz * .8, 1000)
