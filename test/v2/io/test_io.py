...
def test_mat_fit():
    ...
    v2_main(['-D', 'data,D', '--mode', 'fit', fname, fname_out, '--choose', 'baseline', '-p=-2'])

    fname_out2 = gen_outname('out2.yaml')
    v2_main(['-D', 'data,D', '--mode', 'fit', fname_out, fname_out2, '--choose', 'baseline'])

    fname_out3 = gen_outname('out3.mat')
    v2_main(['-D', 'data,D', '--mode', 'fit', fname_out2, fname_out3, '--choose', 'baseline'])

    fname_out4 = gen_outname('out4.h5')
    v2_main(['-D', 'data,D', '--mode', 'fit', fname_out3, fname_out4, '--choose', 'baseline'])

    fname_out5 = gen_outname('out5.pkl')
    v2_main(['-D', 'data,D', '--mode', 'fit', fname_out4, fname_out5, '--choose', 'baseline'])
...
def test_csv_fit():
    ...
    v2_main(['-D', 'data,D', '--mode', 'fit', fname_in, fname_out, '--choose', 'baseline', '-p=-30'])
